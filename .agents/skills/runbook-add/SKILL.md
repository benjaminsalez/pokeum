...
```bash
echo '{"session_id":"test-runbook","tool_response":{"stdout":"<paste a representative slice of the real error output>"}}' | python .Codex/hooks/runbook_matcher.py
```
...
```bash
node .Codex/skills/openwiki/scripts/openwiki-meta.mjs lint
node .Codex/skills/openwiki/scripts/openwiki-meta.mjs record update --pages troubleshooting.md
```
